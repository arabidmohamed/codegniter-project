	...
	class Items_model extends CI_Model{
		...
		// #Add category function
			...
			public function getCategories($data = array()){
				$query = $this->db->order_by('Order_In_List', 'asc')->get(TBL_ITEM_CATEGORIES);
				return $query->result();
			}
			...
			public function getCategoryByID($data = array()){
				...
				$this->db->where($where);
				$query = $this->db->get(TBL_ITEM_CATEGORIES);
				return $query->result();
			}
			...
			public function updateCategory($data = array()){
				$where = array('Category_ID' => $data['Category_ID']);
				$this->db->where($where);
				$query = $this->db->update(TBL_ITEM_CATEGORIES, $data);
				return $query;
			...
			public function deleteCategory($data = array()){
				$where = array('Category_ID' => $data['Category_ID']);
				$this->db->where($where);
				$query = $this->db->delete(TBL_ITEM_CATEGORIES);
				return $query;
		...
		public function addSubCategory($data = array()){
			$this->db->insert(TBL_ITEM_SUBCATEGORIES, $data);
			return  $this->db->insert_id();
		}
		...
		public function getSubCategories($categoryid = 0)
		{
			...
			$query = $this->db
							->select('psc.*')
							->from(TBL_ITEM_SUBCATEGORIES.' as psc')
							->join(TBL_ITEM_CATEGORIES.' as pc', 'pc.Category_ID = psc.Category_ID')
		...
		// 	$query = $this->db
		// 					->select('psc.*')
		// 					->from(TBL_ITEM_CATEGORIES.' as psc')
		...
		public function getSubCategoryByID($data = array()){
			...
			$this->db->where($where);
			$query = $this->db->get(TBL_ITEM_SUBCATEGORIES);
			return $query->result();
		}
		...
		public function updateSubCategory($data = array()){
			$where = array('SubCategory_ID' => $data['SubCategory_ID']);
			$this->db->where($where);
			$query = $this->db->update(TBL_ITEM_SUBCATEGORIES, $data);
			return $query;
		...
		public function deleteSubCategory($data = array()){
			$where = array('SubCategory_ID' => $data['SubCategory_ID']);
			$this->db->where($where);
			$query = $this->db->delete(TBL_ITEM_SUBCATEGORIES);
			return $query;
		...
		public function getProductHistory($Class_ID = 0){
			...
			$product = $this->db
							...
							->join(TBL_ITEM_SUBCATEGORIES.' as psc', 'psc.SubCategory_ID = p.SubCategory_ID')
							->join(TBL_ITEM_CATEGORIES.' as pc', 'pc.Category_ID = psc.Category_ID')
...
*/
			...
			foreach($product as $pd){
				$pd->ProductPictures = $this->db
													...
													->join(TBL_ITEMS.' as p', 'p.Class_ID = pd.Class_ID')
	    ...
	    public function deleteItem($data = array()){
	        ...
	        $this->db->where($where);
	        $query = $this->db->update(TBL_ITEMS, $data);
	        return $query;
	    }
	    ...
	    public function getItemPageName(){
		    return $this->db
		    				->select('*')
	    ...
	    public function deleteProduct($data = array())
	    {
	        ...
	        $this->db->where($where);
	        $query = $this->db->delete(TBL_ITEMS);
	        if ($query) {
	            $where1 = array(
	                'Class_ID' => $data['Class_ID']
	            );
		...
		public function getAllProducts_Excel(){
			return $this->db
							...
							->from(TBL_ITEMS.' as p')
							->join(TBL_ITEM_CATEGORIES.' as c', 'c.Category_ID = p.Category_ID')
