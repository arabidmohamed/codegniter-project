...
<link rel="stylesheet" type="text/css" href="<?=base_url($GLOBALS['acp_css_dir'].'/select2-bootstrap.min.css')?>">
	...
	<div id="content-main">
		<h3><?=getSystemString('edit_installation_price')?></h3>
			<div class="row">
				...
				<div class="col-md-12">
					...
					<form action="<?=base_url($__controller.'/edit_installation_price/').$installation_price_id;;?>" class="form-horizontal" method="post"  enctype="multipart/form-data" data-parsley-validate>
						...
						?>
			              <option   <?= ($installation_price_id == $row->City_ID)?'selected':''; ?> value="<?=$row->City_ID?>">
			              		<?=$row->$cat_nn?>
			              </option>
	              <?PHP
		              }
	              ?>
							...
							<div class="form-group">
								...
								<div class="col-xs-12 col-sm-8 col-md-5 no-padding-left">
									...
									<input type="number" 
											...
											value="<?= $installation_price->Installation_Percentage ?>" 
											placeholder="10" 
											required 
						...
						<div class="form-group">
							<div class="col-xs-12 text-right">
								<input type="submit" class="btn btn-primary" value="<?=getSystemString('edit_installation_price')?>" name="submit"/>
							</div>
						</div>
					</form>
