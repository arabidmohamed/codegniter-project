...
<!-- Header -->
  ...
  <?=   $this->load->view('domain_registration/profile_navigation'); ?>
                    ...
                    <div id="newDomain" class="tab-pane fade in active show">
                        <div class="bs-stepper">
                            ...
                            <div class="bs-stepper-content mt-5">
                                ...
                                <div id="documents" >
                                    <form action="<?= base_url('edit_waiver_review/'.encryptIt($domain_details->Domain_ID)) ?>" enctype="multipart/form-data" method="POST" data-parsley-validate>
                                        <h6 class="form-title hide"><?= getSystemString('speech') ?></h6>
                                        <div class="row no-gutters details khetab hide">
                                            <div class="col-12 mt-4"></div><!-- /.mt-3 -->
                                            <div class="col-md-5 mb-3">
                                                <label class="title-label mb-md-0 mb-4 "><?= getSystemString('speech_a') ?></label>
                                            </div><!-- /.col-md-4 -->
                                            <div class="col-md-3 mb-3">
                                                <a onclick="javascript:print_speech()" href="#!" class="btn btn-block btn-outline-primary" style="font-size: .8rem">
    ...
    <?php    $domain_speech = json_decode($domain_details->Speech_File); ?>
                            ...
                            <input type="file" class="fileBtnField" name="speech_file"  >
                                        ...
                                        <div class="row no-gutters details mt-3">
  ...
  <?php    $domain_support = json_decode($domain->Support_File);?>
                                                ...
                                                <?php foreach ($doc_types as $key => $doc_type) { ?>
                                                    <?php if(($domain_details->TLD == '.sa' || $domain_details->TLD == '.pub.sa' || $domain_details->TLD == '.السعودية') && ($doc_type->id == 78 || $doc_type->id == 80) && ($domain_details->Org_Activity_ID == 69)){ ?>
                                                           <label class="radio-container"><?= $doc_type->$doc_name ?>
                                                            <input <?= ($domain_support->Doc_Type_ID == $doc_type->id)?'checked':'' ?> type="radio" value="<?= $doc_type->id ?>" name="document-type" required data-parsley-required-message="<?=getSystemString('required')?>">
                                                            <span class="radio-checkmark"></span>
                                                        </label>
                                                    <?php } ?>
                                                <?php } ?>
                                                <?php foreach ($doc_types as $key => $doc_type) { ?>
                                                    <?php if(($domain_details->TLD == '.sa' || $domain_details->TLD == '.com.sa' || $domain_details->TLD == '.net.sa' || $domain_details->TLD == '.السعودية' || $domain_details->TLD == '.org.sa' || $domain_details->TLD == '.edu.sa' || $domain_details->TLD == '.med.sa' || $domain_details->TLD == '.sch.sa') && ($doc_type->id != 78 && $doc_type->id != 80) && ($domain_details->Org_Activity_ID != 69)){ ?>
                                                           <label class="radio-container"><?= $doc_type->$doc_name ?>
                                                            <input <?= ($domain_support->Doc_Type_ID == $doc_type->id)?'checked':'' ?> type="radio" value="<?= $doc_type->id ?>" name="document-type" required data-parsley-required-message="<?=getSystemString('required')?>">
                                                            <span class="radio-checkmark"></span>
                                                        </label>
                        ...
                        <div class="issuer_list   row  col-12  <?= ($domain_support->Doc_Type_ID != 77)?'hide':'' ?>">
                        ...
                        <select class="form-control select"
                                        ...
                                        foreach($doc_issures as $row){
                                            ?>
                    <option <?= ($row->Doc_Issures_ID ==  $domain_support->Doc_Issures_ID)?'selected':'' ?>  value="<?=$row->Doc_Issures_ID?>"><?=$row->$issuer_name?></option>
                                            <?PHP
                                        }
                                    ?>
                                        </select>
           ...
           <div class="row no-gutters step-2">
                                ...
                                <div class="doc_title" style="display: none;">
                                            ...
                                            <div class="col-md-9 mb-3">
                                                <input value="<?= $domain_support->Doc_Title ?>" type="text" name="doc_title" placeholder="<?= getSystemString('doc_title') ?>"  data-parsley-required-message="<?=getSystemString('required')?>">
                                            </div><!-- /.col-md-4 -->
                                </div>
                                            ...
                                            <div class="col-md-9 mb-3">
                                                <input value="<?= $domain_support->Doc_Num ?>"  type="text" name="doc_number" placeholder="<?= getSystemString('doc_number') ?>" data-parsley-type="digits" required data-parsley-required-message="<?=getSystemString('required')?>">
                                            </div><!-- /.col-md-4 -->
                                            <div class="col-12 mt-4"></div><!-- /.mt-3 -->
                                            <div class="col-lg-2 col-md-3 mb-3">
                                            ...
                                            <div class="col-md-9 mb-3">
                                                <input value="<?= $domain_support->Doc_Date ?>" type="text" autocomplete="off" name="doc_date" class="datepicker" placeholder="dd/mm/yyyy"  required data-parsley-required-message="<?=getSystemString('required')?>">
                                            </div><!-- /.col-md-4 -->

                                            <input type="hidden" name="hijri_date" class="hijri_date" value="<?= $domain_support->Hijri_Date ?>">
                                            <input type="hidden" name="meladi_date" class="meladi_date"  value="<?= $domain_support->Meladi_Date ?>">
                                          ...
                                          <div class="col-12 mt-4">
                                            ...
                                            <div class="col-md-3 mb-3">
                                                ...
                                                <div class="fileBtn">
                                                    ...
                                                    <input type="file" class="fileBtnField" name="support_file" <?= (empty($domain_support))?'required':'' ?>  data-parsley-required-message="<?=getSystemString('required')?>">
                                                 <span class="text-muted file-name support_file"></span>
                                                <span class="text-muted file-name">
                                                          <?php if(!empty($domain_support)){ ?>
                                                     <img src="<?=base_url('style/site/assets/')?>images/pdf.svg" alt="">
                                        ...
                                        <div class="row">
                                            <div class="col-md-3 mb-3">
                                                <a href="<?= base_url('domain_waiver/'.encryptIt($domain_details->Domain_ID)) ?>" class="btn btn-primary-inverse btn-block prev"><?= getSystemString('previous') ?></a><!-- /.btn btn-primary btn-block -->
                                            </div><!-- /.col-md-3 -->
...
<?PHP
    ...
    $speech_url = base_url('edit_speech_pdf/'.encryptIt($domain_details->DW_Domain_ID));
