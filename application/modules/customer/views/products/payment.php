...
<?= $this->load->view('domain_registration/profile_navigation'); ?>
                ...
                <div id="main-dashboard">
                    ...
                    <div class="bs-stepper mb-5">
                            ...
                            <div class="col-lg-8"> 
                                <div class="card border-0"> 
                                    <div class="form-group row">
                                        <div class="col-12">
                                            <h5 class="title mb-4"><b><?= getSystemString('Please choose a payment method') ?></b></h5>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <div class="col-md-12">
                                            <div class="payment-way-box ">
                                                ...
                                                <input type="radio" name="payment" id="payment-2" class="input-hidden" value="MADA" required="" autocomplete="off">
                                                ...
                                                <input type="radio" name="payment" id="payment-3" class="input-hidden" value="MASTER" autocomplete="off">
                                                ...
                                                <input type="radio" name="payment" id="payment-4" class="input-hidden" value="VISA" autocomplete="off">
                                                ...
                                                <input type="radio" id="payment-5"  name="payment" value="AMEX" class="input-hidden" autocomplete="off">
                                                <label for="payment-5" class="payment-way">
                                                ...
                                                <?php if($can_add_transaction){ ?> 
                                                    <input type="radio" id="payment-6"  name="payment-method" value="WALLET" class="input-hidden" autocomplete="off">
                                                    <label for="payment-6" class="payment-way flex-column">
                                    ...
                                    <div class="way-box MADA-way" id="payment_response">
                            ...
                            <div class="col-lg-4"> 
                                <div class="card mb-4">
                                    <div class="card-body p-4">  
                                        <div class="">
                                            <h5 class="title mb-4"><b><?= getSystemString('payment details') ?></b></h5>
                                        </div>
                                        <div class="details-table">
                                            <table class="table table-responsive table-course"> 
                                                <tbody>
                                                    <tr>
                                                        <td class="border-top-0" scope="row"><?= getSystemString('amount') ?></td>
                                                        <td class="border-top-0 text-right"><?= $sub = $price->Price * $subscription->Num_of_licenses ?> <?= getSystemString('SAR') ?></td>
                                                    </tr>  
                                                    <tr>
                                                        <td scope="row"><?= getSystemString('VAT') ?> (<?= $settings['web_settings'][0]->Vat ?>%)</td>
                                                        <td class="text-right"><?= $vat = round(($settings['web_settings'][0]->Vat / 100) * $sub, 2) ?> <?= getSystemString('SAR') ?></td>
                                                    </tr>
                                                    <tr>
                                                        <th scope="row"><?= getSystemString('total amount') ?></th>
                                                        <th class="text-right"><?= $vat + $sub ?> <?= getSystemString('SAR') ?></th>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                        ...
                        <div class="row justify-content-between"> 
                            <div>
                                <a href="<?= base_url("products/subscription/".encryptIt($subscription->Product_ID)) ?>" class="btn btn-primary-inverse px-4"> <?= getSystemString('back') ?></a> 
                                <a href="<?= base_url('products/list') ?>" class="btn btn-light px-3"> <?= getSystemString('cancel') ?></a> 
                            </div>
                            <button type="button" id="pay" class="btn btn-primary-inverse px-5"> <?= getSystemString('next') ?></button> 
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        $(".payment-way-box input[type='radio']").click(function () {
            if ($(this).val() !== '') {
                let payment_type = $(this).val();
                $.post("<?= base_url('products/checkout/' . encryptIt($rId)) ?>",
                        {
                            '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                            payment_type: payment_type,
                        },
                        function (data, status) {
                            $('#payment_response').html(data);
                        });
            }
