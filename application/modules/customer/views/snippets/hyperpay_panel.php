         ...
         <?php if($cart_type == 'WALLET'){ ?>
                   <div class="row mb-5">
                     ...
                     <?php }else{ ?>
                          ...
                          <a class="wpwl-button wpwl-button-pay" href="<?=base_url("payment_success").'?id='.$checkout_id?>" ><?= getSystemString('pay_now') ?></a>
          ...
          <div class="panel-body">
           ...
           <?php if($cart_type == 'MADA'){ ?>
              ...
              <?php }elseif($cart_type == 'MASTER'){ ?>
                <form action="<?=base_url("payment_success")?>" class="paymentWidgets" data-brands="MASTER"></form>
              <?php }elseif($cart_type == 'AMEX'){ ?>
                <form action="<?=base_url("payment_success")?>" class="paymentWidgets" data-brands="AMEX"></form>
              <?php }else{ ?>
                <form action="<?=base_url("payment_success")?>" class="paymentWidgets" data-brands="VISA"></form>
              <?php } ?>
