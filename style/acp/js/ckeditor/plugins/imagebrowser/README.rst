...
**imagebrowser** is a `CKEditor <https://ckeditor.com/>`_ plugin that allows images on the server to be browsed and picked
for inclusion into the editor's contents.

This plugin integrates with the **image** plugin (part of CKEditor),
by making it provide a **Browse Server** button in the Image dialog window (`screenshot here <https://ckeditor.com/sites/default/files/styles/large/public/image/image_manager.png>`_).

The way you use it is very similar to `imageGetJson <https://imperavi.com/redactor/docs/settings/#set_imageGetJson>`_ in `Redactor <https://imperavi.com/redactor/>`_
...
Make sure you're using the **Standard** or **Full** `CKEditor packages <https://ckeditor.com/download>`_.
The **Basic** package lacks an in-built "File Browser" plugin, which this plugin depends on.
You can also use a `Custom CKEditor package <https://ckeditor.com/builder>`_, if you build it with "File Browser" plugin support.
